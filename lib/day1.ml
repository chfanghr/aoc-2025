...
let parser =
  ...
  let uint =
    let* s = take_while1 (function '0' .. '9' -> true | _ -> false) in
    let* i =
      Option.value_map (Int.of_string_opt s)
        ~default:(fail "failed to parse int")
        ~f:return
    in
    return i
  in
  let rot = char 'L' *> (Int.neg <$> uint) <|> char 'R' *> uint in
  let newline = char '\n' in
  sep_by1 newline rot
